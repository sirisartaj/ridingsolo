...
class AdminRepository
{
  ...
  {      
    try {
      $sql = "SELECT * FROM  sg_users where (user_status =! '9' or user_status is NULL) ";
  ...
  public function Approverejectuser($data) {    
    try {
      extract($data);
      $sql = "UPDATE sg_users SET ustatus=:ustatus, modified_by=:modified_by,modified_date=:modified_date WHERE user_id = :user_id";
      $stmt = $this->connection->prepare($sql);  
      $stmt->bindParam(":user_id",$user_id);
